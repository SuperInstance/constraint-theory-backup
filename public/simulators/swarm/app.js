...
class Boid {
    ...
    draw(ctx, params, colorCache) {
        ...
        // Note: Removed shadowBlur for 5-10x performance improvement
        // The glow effect was causing significant GPU overhead
...
class BoidSimulation {
    constructor() {
        ...
        this.fps = 60;
        this.colorModes = ['velocity', 'group', 'depth', 'uniform'];
    ...
    setupKeyboardShortcuts() {
        document.addEventListener('keydown', (e) => {
            ...
            switch(e.key.toLowerCase()) {
                ...
                case '3':
                case '4':
                    this.setColorMode(parseInt(e.key) - 1);
                    break;
            }
        });
    }
    ...
    setupTouchSupport() {
        ...
        this.canvas.addEventListener('touchstart', (e) => {
            ...
            if (this.params.obstacleMode) {
                ...
                // Toggle predator mode on touch start
                this.mousePos.x = x;
                this.mousePos.y = y;
                this.params.predatorMode = true;
                document.getElementById('predatorMode').checked = true;
            }
        }, { passive: false });

        this.canvas.addEventListener('touchend', (e) => {
            // Optionally disable predator mode when touch ends
            // this.params.predatorMode = false;
            // document.getElementById('predatorMode').checked = false;
        });
    }
    ...
    togglePause() {
        ...
        if (pauseIndicator) {
            pauseIndicator.style.display = this.isPaused ? 'flex' : 'none';
        }
        // Update button state
        const pauseBtn = document.getElementById('pauseBtn');
        if (pauseBtn) {
            pauseBtn.setAttribute('aria-pressed', this.isPaused);
            pauseBtn.innerHTML = this.isPaused ? '▶️ Resume Animation' : '⏸️ Pause Animation';
        }
    }

    togglePredatorMode() {
        this.params.predatorMode = !this.params.predatorMode;
        document.getElementById('predatorMode').checked = this.params.predatorMode;
    }

    setColorMode(index) {
        if (index >= 0 && index < this.colorModes.length) {
            this.params.colorMode = this.colorModes[index];
            document.getElementById('colorMode').value = this.colorModes[index];
        }
    }
